...
def extract(text: str) -> dict:
    ...
    action_verbs = (
        "add", "bulk", "categorize", "clear", "create", "delete", "migrate",
        "move", "new", "remove", "rename", "save", "set", "share", "uncheck",
        "unshare", "update",
    )
    ...
    handlers = set(
        ...
        for m in re.finditer(
            rf'(["\'])({verb_pattern}[a-z0-9]*(?:-[a-z0-9]+)+)\1', text
        )
    )
...
def main() -> None:
    p=argparse.ArgumentParser();p.add_argument("app_js",type=Path);p.add_argument("--schema",type=Path);p.add_argument("--out",type=Path,required=True);a=p.parse_args()
