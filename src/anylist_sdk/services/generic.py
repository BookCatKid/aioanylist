...
class GenericDomainService(OperationService):
    ...
    def __init__(self, transport: AnyListTransport, state: AnyListState, *, user_id: str,
                 queue_id: str, endpoint: str, operation_type: str, operation_list_type: str,
                 journal=None) -> None:
        super().__init__(transport, state, user_id=user_id,
            spec=QueueSpec(queue_id, endpoint, operation_type, operation_list_type), journal=journal)
