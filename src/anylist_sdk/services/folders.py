...
class FoldersService(OperationService):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            user_id=user_id,
            spec=QueueSpec(
                f"{user_id}:list-folders",
                "/data/list-folders/update",
                "PBListFolderOperation",
    ...
    def all(self) -> list[PBListFolder]:
        return list(self.state.list_folders.values())

    def get(self, folder_id: str) -> PBListFolder | None:
        return self.state.list_folders.get(folder_id)

    def has_pending_delete_items(self) -> bool:
        return any(
            str(op.metadata.handlerId) == "delete-folder-items" for op in self.queue._pending
        )
    ...
    async def create(
        ...
        *,
        ...
        if not parent_id:
            raise RuntimeError("No root folder synchronized")
        ...
        await self.operation(
            "create-new-folder", listFolder=folder, updatedParentFolderId=parent_id, flush=flush
        )
    ...
    async def rename(self, folder_id: str, name: str, *, flush: bool = True) -> None:
        ...
        await self.operation(
            "set-folder-name",
            listFolder=PB.PBListFolder(identifier=folder_id, name=name),
            flush=flush,
        )

    async def set_hex_color(self, folder_id: str, color: str, *, flush: bool = True) -> None:
        f = self._require(folder_id)
        f.folderSettings.folderHexColor = color
        p = PB.PBListFolder(identifier=folder_id)
        p.folderSettings.folderHexColor = color
    ...
    async def set_lists_sort_order(self, folder_id: str, value: int, *, flush: bool = True) -> None:
        f = self._require(folder_id)
        f.folderSettings.listsSortOrder = value
        p = PB.PBListFolder(identifier=folder_id)
        p.folderSettings.listsSortOrder = value
        await self.operation("set-lists-sort-order", listFolder=p, flush=flush)

    async def set_folder_sort_position(
        self, folder_id: str, value: int, *, flush: bool = True
    ) -> None:
        f = self._require(folder_id)
        f.folderSettings.folderSortPosition = value
        p = PB.PBListFolder(identifier=folder_id)
        p.folderSettings.folderSortPosition = value
        await self.operation("set-folder-sort-position", listFolder=p, flush=flush)

    async def reorder(
        self, folder_id: str, items: Sequence[PBListFolderItem], *, flush: bool = True
    ) -> None:
        f = self._require(folder_id)
        del f.items[:]
        for x in items:
            f.items.add().CopyFrom(x)
        await self.operation(
            "set-ordered-folder-items",
            originalParentFolderId=folder_id,
            folderItems=[clone_message(x) for x in items],
            flush=flush,
        )

    async def move(
        self,
        items: Sequence[PBListFolderItem],
        original_parent_id: str,
        updated_parent_id: str,
        *,
        flush: bool = True,
    ) -> None:
        ...
        kept = [
            ...
            if (int(x.itemType), str(x.identifier)) not in keys
        ]
        del original.items[:]
        for item in kept:
        ...
        await self.operation(
            "move-folder-items",
            folderItems=wire_items,
            originalParentFolderId=original_parent_id,
            updatedParentFolderId=updated_parent_id,
            flush=flush,
        )

    async def delete_items(
        self, items: Sequence[PBListFolderItem], parent_id: str, *, flush: bool = True
    ) -> None:
        ...
        await self.operation(
            "delete-folder-items",
            folderItems=wire_items,
            originalParentFolderId=parent_id,
            flush=flush,
        )
